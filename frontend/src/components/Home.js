...
const OpsLayout = ({ name }) => {
  ...
  const validateForm = () => {
    ...
    if (!formData.areaDeTrabajo)
      newErrors.areaDeTrabajo = "El área de trabajo es requerida";
    ...
    if (!formData.nombrePaciente)
      newErrors.nombrePaciente = "El nombre del paciente es requerido";
    if (!formData.edadPaciente)
      newErrors.edadPaciente = "La edad del paciente es requerida";
    if (!formData.nombreCirujano)
      newErrors.nombreCirujano = "El nombre del cirujano es requerido";
    if (!formData.dniPaciente)
      newErrors.dniPaciente = "El DNI del paciente es requerido";
  ...
  const filteredOps = ops.filter(
    (op) => op.dniPaciente.includes(searchDni) && op.nombre === name
  );
  ...
  return (
    <Container fluid className="p-0">
      ...
      <Row className="mb-3 justify-content-center mx-0">
        <Col xs="auto">
      ...
      <div id="listOpsTable" className="table-responsive px-0">
        <ListOps
          ops={filteredOps}
          isLoading={isLoading}
